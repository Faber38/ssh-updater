...
python -c 'import pathlib, sys; expected = pathlib.Path("release-python.txt").read_text(encoding="utf-8").strip(); actual = ".".join(map(str, sys.version_info[:2])); sys.exit(0 if actual == expected else "Bitte Release-Umgebung mit Python " + expected + ".x neu erstellen.")'
